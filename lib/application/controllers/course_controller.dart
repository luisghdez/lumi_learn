...
Future<void> completeCurrentLesson() async {
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // ✅ Regardless of increment, a successful lesson is a check-in
      // Schedule the risk reminder at now + 22h (2h before a 24h loss window)
      await NotificationCampaigns.instance()
          .scheduleStreakRiskReminderAfterLastCheckIn(
        lastCheckIn: DateTime.now(),
      );

      // (Optional) If you want a nightly study reminder too:
      // await NotificationCampaigns.instance()
      //     .scheduleDailyReminder(DateTime(0, 0, 0, 21, 0)); // 9 PM

      // Navigate to result screen after a tiny delay + sound
      Future.delayed(const Duration(milliseconds: 300), () {
        playLessonCompleteSound();
        Get.offAll(() => LessonResultScreen(
              backgroundImage: activePlanet.value!.backgroundPaths.first,
